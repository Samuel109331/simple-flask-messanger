...
def createAccount(fullname,username,birthdate,gender,country,avatar,password):
    with pymysql.connect(host="sql.freedb.tech",user="freedb_sami9644",password="Hma8#vCBBUD&!Y8",database="freedb_messangerpro_db") as conn:
        cur = conn.cursor()
        time = datetime.now()
        ...
        cur.execute("INSERT INTO users(fullname,username,birthdate,gender,country,avatar,password,session,createdon)\
                    VALUES(%s,%s,%s,%s,%s,%s,%s,false,%s)",(fullname,username,birthdate,gender,country,avatar,password,ultimatetime))
        conn.commit()
